...
class PagerDutyHookTests(WebhookTestCase):
    ...
    def test_valid_signature(self) -> None:
        ...
        # Get the webhook secret from the API key
        api_key = self.get_api_key(self.test_user)
    ...
    def test_invalid_signature(self) -> None:
        ...
        # Use an intentionally wrong signature
        invalid_signature = "0" * 64  # Invalid 64-character hex string
        ...
        self.assert_json_error(result, "Invalid webhook signature")
    ...
    def test_multiple_signatures_one_valid(self) -> None:
        ...
        # Mix valid signature with invalid ones (comma-separated format)
        invalid_signature = "0" * 64
        mixed_signatures = f"v1={invalid_signature},v1={valid_signature}"
        
        # Send webhook with mixed signatures
        result = self.client_post(
            self.url,
            payload,
            content_type="application/json",
    ...
    # NEW TESTS FOR SERVICE EVENTS (V3)
    def test_service_created_v3(self) -> None:
        """Test service.created event from PagerDuty V3"""
        expected_message = "Service [Test Service](https://pig208.pagerduty.com/services/PSERVICE1) created."
        self.check_webhook("service_created_v3", "Incident Test Service", expected_message)

    def test_service_updated_v3(self) -> None:
        """Test service.updated event from PagerDuty V3"""
        expected_message = "Service [Test Service](https://pig208.pagerduty.com/services/PSERVICE1) updated."
        self.check_webhook("service_updated_v3", "Incident Test Service", expected_message)

    def test_service_deleted_v3(self) -> None:
        """Test service.deleted event from PagerDuty V3"""
        expected_message = "Service [Test Service](https://pig208.pagerduty.com/services/PSERVICE1) deleted."
        self.check_webhook("service_deleted_v3", "Incident Test Service", expected_message)
